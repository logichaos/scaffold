...
- Q: What default security posture should the scaffold enforce for new projects? → A: Hardened Production

- Q: Which performance benchmarks should the scaffold include? → A: Both (build/test budgets + runtime benchmarks)

- Q: What observability scaffolding should be generated with the scaffolded solution? → A: Full Observability

- Q: If CI exceeds the build+test time budget, what should happen by default? → A: Warn only
...
### Non-Functional Additions (Security & Performance)

- **NFR-SEC-001**: Default security posture: Hardened Production — scaffold generates SBOM, SCA CI integration, secret scanning, code-signing templates, and runtime hardening guidance.
- **NFR-SEC-002**: CI rules generated by the scaffold MUST fail builds for Critical or High vulnerabilities until an explicit exception is recorded in the repository.
- **NFR-SEC-003**: The scaffold MUST include pre-commit and CI secret scanning configuration and a documented secret handling procedure in the generated `README.md`.

- **NFR-PERF-001**: Include CI build and test time budgets: default max build+test time target 5 minutes for scaffolded solution; CI should flag or fail if exceeded.
- **NFR-PERF-002**: Provide optional runtime benchmark templates (sample minimal API and harness) with suggested targets: p95 <200ms, p99 <500ms, and a basic RPS target (e.g., 1000 RPS for simple endpoints) — included as templates, enabled opt-in.
- **NFR-PERF-003**: Include flaky test detection hooks (e.g., re-run fails or mark tests with instability) and CI alerts when test flakiness is detected.

- **NFR-OBS-001**: Observability defaults: Full Observability — include OpenTelemetry tracing, Prometheus metrics endpoint, and example Grafana dashboard templates (as scaffolding), plus CI guidance for metric thresholds/alerts.
